...
export default function DomeGallery({
  ...
  const autoRotateRAF = useRef(null); // 자동 회전 애니메이션 ID 저장용
  ...
  const items = useMemo(() => {
    // 백엔드에서 받은 데이터가 없으면 기본 이미지를 보여주거나 빈 배열 처리
    const pool = images && images.length > 0 ? images : DEFAULT_IMAGES;
    return buildItems(pool, segments);
  }, [images, segments]);
  ...
  useEffect(() => {
    const rotate = () => {
        // 사용자가 드래그 중이 아닐 때만 자동으로 회전
        if (!draggingRef.current && !openingRef.current) {
        // y축 값을 조금씩 증가 (0.1이 속도입니다. 더 천천히 하려면 0.05)
