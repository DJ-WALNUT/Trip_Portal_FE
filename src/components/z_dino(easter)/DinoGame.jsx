...
const GRAVITY = 0.6;          // 중력 (매 프레임마다 아래로 당기는 힘)
const JUMP_STRENGTH = 10;     // 점프력 (위로 솟구치는 힘)
const GAME_SPEED_START = 5;   // 초기 게임 속도
const GAME_SPEED_MAX = 100;    // 최대 게임 속도
const ACCELERATION = 0.2;   // 가속도 (프레임마다 빨라지는 정도)

const DinoGame = ({ isOpen, onClose }) => {
  // === React State (UI 렌더링용) ===
  ...
  const gameState = useRef({
    ...
    gameSpeed: GAME_SPEED_START, // 현재 게임 속도
  ...
  const gameLoop = useCallback(() => {
    ...
    if (state.isJumping || state.dinoY > 0) {
      ...
      state.dinoVelocity -= GRAVITY;           // 중력만큼 속도 감소
    }
      ...
      <div className="dino-card">
        <div className="dino-header">
          <div className="dino-title">
            <span className="badge">이스터에그</span>
            <h3>달려라 엔지니!</h3>
          </div>
        ...
        >
          ...
          {isPlaying && (
            <div 
              ...
              className="obstacle"
            >
              <Bug size={34} color="#e74c3c" strokeWidth={2.5} />
            </div>
          )}
        </div>
